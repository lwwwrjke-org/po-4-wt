function TASK_2() {
    ...
    while (true) {
        ...
        if (ToInt(floor) && floor >= 1 && floor <= 25) { break; }
        else { alert("Введите целое число от 1 до 25") }
    }
    
    while (true) {
        var entrance = prompt("Количество подъездов? [1-10] ");
        if (ToInt(entrance) && entrance >= 1 && entrance <= 10) { break; }
        else { alert("Введите целое число от 1 до 10") }
    }

    while (true) {
        var apartments = prompt("Количество квартир на этаже? [1-20] ");
        if (ToInt(apartments) && apartments >= 1 && apartments <= 20) { break; }
        else { alert("Введите целое число от 1 до 20") }
    }
