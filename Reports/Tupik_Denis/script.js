...
const LFname = document.querySelector(".name");
...
const showData = () => {
    fetchUsers(search.value).then((response) => {
        ...
        LFname.innerHTML = `<div>${response.profileData.name}</div>`;
        ...
        avatar.innerHTML = `<img class="profile-img" src="${response.profileData.avatar_url}"/>`;

        let html = "<p class=\"pop\">Popular public repositories:</p>";
        html += "<table>";
        ...
        for (let i = 0; i < response.reposData.length; i++){
            ...
            if (i < response.reposData.length && (i + 1 == response.reposData.length)){
                html += "<tr>";
                html += `<td><div class="input_td"><p class="name_rep">${response.reposData[i].name}</p><p class="describe">${response.reposData[i].description}</p><br></div></td>`;
                html += "</tr>";
                break;
            }
            ...
            if (i % 2 == 0){
                html += "<tr>";
                html += `<td><div class="input_td"><p class="name_rep">${response.reposData[i].name}</p><p class="describe">${response.reposData[i].description}</p><br></div></td>`;
                html += `<td><div class="input_td"><p class="name_rep">${response.reposData[i + 1].name}</p><p class="describe">${response.reposData[i + 1].description}</p><br></div></td>`;
                html += "</tr>";
            }
        }
